...
> **Scope note:** The original R version only ran the Step 2-4 deep classification on **D1**; D2 and D3 only got the Step 1 counts plus a manually written conclusion, with no classification code. This Python version mirrors that scope exactly — D2/D3 aren't shortchanged, that's genuinely how much analysis the original project did on them. Step 6's `CLEANING_RULES` still covers all three datasets, since that's a straight translation of the original hand-written conclusions into a structured config, not new analysis.
